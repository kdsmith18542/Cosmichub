...
return function ($pdo) {
    ...
    if (!$tableExists) {
        // Create user_tokens table with minimal structure first
        ...
        // Create indexes
        $pdo->exec("CREATE INDEX IF NOT EXISTS idx_user_tokens_type ON user_tokens (type)");
        $pdo->exec("CREATE INDEX IF NOT EXISTS idx_user_tokens_expires_at ON user_tokens (expires_at)");
        $pdo->exec("CREATE INDEX IF NOT EXISTS idx_user_tokens_used_at ON user_tokens (used_at)");
